    ...
    public void PawnMovePlate(int x, int y)
    {
        ...
        if (sc.PositionOnBoard(x, y))
        {
            if (sc.GetPosition(x, y) == null)
            {
                ...
                if (player == "white" && sc.PositionOnBoard(x, y + 1) && sc.GetPosition(x, y + 1) == null && yBoard == 1)
                { 
                    MovePlateSpawn(x, y + 1);
                }

                if (player == "black" && sc.PositionOnBoard(x, y  -1) && sc.GetPosition(x, y  -1) == null && yBoard == 6)
                {
                    MovePlateSpawn(x, y - 1);
                }
            }
